...
As a group and using Git/Github, write a program that plays the women’s soccer season as defined in A5. Please review that previous assignment.

Modify the code to use functions as defined below. You can do more than what is required.

Everyone works on a Main function that calls other functions. Create the following functions:

NOTE: We want at least 4 functions and we definitely want the functions that get the home team name and the opponent name (removing the home team name). After that, make sure the game works. You can use your creativity to get this completed. Once again, you can choose the functions you create so you do not need to reach out ahead of time for approval.

Remember, a function is used if you think you might reuse the code another time in the program or if it allows you to divide work into simpler blocks of code to maintain.

Look at these directions and you can determine some functions you can create (i.e. menu for one)
...
Upload the github link for your project (so it should be public) to Learning Suite.

Make sure there are comments in the code including all the team member names.
